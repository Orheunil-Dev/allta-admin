...
import {
  useCouponControllerDeleteCouponInfo,
  useCouponControllerGetCouponInfoDetail,
  useCouponControllerUpdateCouponInfo,
} from "@/api/coupon/coupon";
import { UpdateCouponInfoRequest } from "@/api/models";
...
export default function CouponRegister() {
  ...
  const [couponInfo, setCouponInfo] = useState<UpdateCouponInfoRequest>({
    id: id as string,
    name: "",
    type: "PROMOTION",
    passType: null,
  ...
  } = useCouponControllerGetCouponInfoDetail(id as string, {
    query: {
      enabled: !!id,
    },
  });
  ...
  // 쿠폰 업데이트 API
  const {
    mutate: updateCouponInfo,
    isPending: updateCouponInfoLoading,
    isError: updateCouponInfoError,
  } = useCouponControllerUpdateCouponInfo();

  // 쿠폰 삭제 API
  const {
    mutate: deleteCouponInfo,
    isPending: deleteCouponInfoLoading,
    isError: deleteCouponInfoError,
  } = useCouponControllerDeleteCouponInfo();
  ...
  const handleUpdateCouponInfo = () => {
    updateCouponInfo(
      ...
      },
      {
        ...
        onError: (error: any) => {
          return alert(
            error.message ?? "쿠폰 정보 수정 중 오류가 발생했습니다."
          );
        },
      }
    );
  ...
  const handleDeleteCouponInfo = () => {
    deleteCouponInfo(
      ...
      },
      {
        ...
        onError: (error: any) => {
          return alert(error.message ?? "쿠폰 삭제 중 오류가 발생했습니다.");
        },
      }
    );
  };
  ...
  useEffect(() => {
    if (couponInfoData) {
      const data = couponInfoData.data;
